...
const Numbers = () => {
    const stats = [
        { number: 100, text: "Fast & Efficient", isPercentage: true },
        { number: 100, text: "Reliable Support", isPercentage: true },
    ...
    return (
        <div ref={sectionRef} className="bg-black py-6 pb-10 lg:py-14 lg:pb-14 xl:container mx-auto">
                    ...
                    {stats.map((item, index) => (
                        <SwiperSlide key={index}>
                            <div className="text-white text-center p-6">
                                <h1 className="text-[50px]  font-cinzel font-extrabold">
                                    {item.number}
                                    {item.isPercentage ? "%" : "+"}
                                </h1>
                ...
                {stats.map((item, index) => (
                    <div key={index} className={`flex justify-center w-full ${index !== stats.length - 1 ? 'border-r-[1px] border-white' : ''}`}>
                        <div className="text-center">
                            <h1 className="text-[30px] lg:text-[50px] font-extrabold font-cinzel">
                                {counts[index]}
                                {item.isPercentage ? "%" : "+"}
                            </h1>
                            <p className="font-georgia">{item.text}</p>
